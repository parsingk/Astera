  ...
  const create = async (): Promise<void> => {
    ...
    try {
      const reply = await window.api.orch.command(projectPath, 'run-create', {
        ...
        // 않는다(스펙 5절).
        //
        // 한때 예약에는 보내지 않았다. 이유는 맞았지만 막는 자리가 틀렸다: 예약의 계획은 스스로
        // 돌지 않고, 그것을 지키는 것은 run-create 다 — 예약이면 계획에 `autoDispatch` 를 주지
        // 않는다. 그런데 `pendingStart`("아직 시작하지 않았다")도 같은 깃발을 타고 있어서, 보내지
        // 않으면 예약 템플릿에 **'실행' 버튼이 아예 붙지 않았다** — 실제로 그렇게 보고됐다.
        //
        // 발화가 만든 회차는 `jobs run` 의 회차처럼 시작한다(U1). 이 폼은 언제나 코디네이터 계정을
        // 보내므로 회차마다 제 코디네이터가 뜬다(run-spawn). 코디네이터 계정이 없는 예약은 CLI 로만
        // 만들어지고, 그 회차는 앱이 배치한다(JobRun.autoDispatch).
