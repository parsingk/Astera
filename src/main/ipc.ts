  ...
  const bootOrch = async (): Promise<void> => {
          ...
          for (const slot of slots) {
            ...
            try {
              ...
              if (run.worktree === undefined) {
                try {
                  // forkWorktree(integrateGit.ts)가 프로젝트가 **서 있는 브랜치**에서 갈라 준다. raw
                  ...
                  const created = await forkWorktree(
                    ...
                    { registry: core.worktrees, log: orchLog }
                  )
              ...
              if (merges.length > 0) {
                ...
                const integration = await integrateWorktrees(runRoot, merges, {}, gitCtx)
    ...
    const deps: OrchServerDeps = {
      ...
      mergeWorktrees: async (runCwd, paths) => {
        ...
        const r = await integrateWorktrees(runCwd, alive, { reap: false }, gitCtx)
