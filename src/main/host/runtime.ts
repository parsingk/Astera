 ...
 * half-written `node.exe` behind would be believed by every later launch — `exists` is the only
...
export function prepareHostRuntime(a: {
  ...
  if (!fs.exists(w.join(shipped, 'node.exe'))) {
    ...
    return { ready: false, did: 'nothing' }
  }
  ...
  return { ready: fs.exists(paths.exePath) && fs.exists(paths.entryPath), did }
}
