...
describe('RunManager', () => {
  it('start는 PTY를 스폰하고 running 상태·onData 전파를 만든다', () => {
    ...
    const st = mgr.start({ projectPath: 'D:/p', projectName: 'p', config: cfg })
  ...
  it('같은 프로젝트 동시 실행은 거부한다', () => {
    const { mgr } = setup()
    mgr.start({ projectPath: 'D:/p', projectName: 'p', config: cfg })
    expect(() => mgr.start({ projectPath: 'D:/p', projectName: 'p', config: cfg })).toThrow(/ALREADY_RUNNING/)
  })

  it('다른 프로젝트는 동시 실행되고 listActive에 함께 나온다', () => {
    const { mgr } = setup()
    mgr.start({ projectPath: 'D:/a', projectName: 'a', config: cfg })
    mgr.start({ projectPath: 'D:/b', projectName: 'b', config: cfg })
