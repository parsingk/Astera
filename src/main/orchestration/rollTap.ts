...
export class OrchRollTap {
   ...
   *
   *  **에피소드보다 오래 살 수 없다.** 지우는 자리가 넷이다: 'none'·'stalled' 게시, 'nudged' 로 오는
   *  제자리 재개, 그리고 재키잉이 확정된 onRolled. 뒤의 둘 — 재개가 실제로 기록되는 자리 — 이
   *  필요한 이유는 onRollState 의 주석에 적었다(건너뛰어지는 'none' 하나가 이 표시를 영구히
   *  남게 만들었고, 그러면 그 세션의 다음 정지가 전부 기록되지 않는다).
   *
   ...
   *  지운다** — 'nudged' 는 아래에서, 계정이 바뀌는 재개는 onRolled 에서. 그 둘이 "이 정지는 이미
   *  기록했다" 라는 조건을 확실히 끝내는 사건이고, 건너뛰어질 수 있는 'none' 은 모두 그 뒤에 온다.
   *
   *  **던지지 않는다** — 부르는 쪽은 롤링의 send 탭이고, 거기서 예외가 새면 롤 자체가 막힌다.
