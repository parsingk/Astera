 ...
 *   `HOST_STOP_BOUND_MS` is logged, and the stop degrades to today's "exit read as a result": the app
 *   kills the run, so the button always stops it.
...
export async function stopRunFromPanel(a: {
  ...
  if (a.isValidation && a.hostDrives) {
    const answer = await bounded(
      a.askHost(a.runId),
      a.boundMs ?? HOST_STOP_BOUND_MS,
    );
    if (answer.kind === "answered" && answer.reply.status === 200) {
      if ((answer.reply.body as { stopped?: unknown } | null)?.stopped === true)
        return;
    } else
      a.log(
        ...
        } — the app stops the run itself, and its exit is read as a result`,
      );
  }
