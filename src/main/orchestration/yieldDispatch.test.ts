...
import { appDiscardRunWorktree, appTimerTick, stopRunFromPanel, HOST_STOP_BOUND_MS } from './yieldDispatch'
import { hostSpeaksDispatch } from '../host/outdated'
import { createDispatchLoop, type DispatchLoopContext } from '../../core/orchestration/exec/dispatchLoop'
...
/** The S4 Host, after `markUnresponsive` (review I1): it still drives, so the app still yields. */
const S4_UNRESPONSIVE = { connected: false, unresponsive: true, features: S4_HOST.features }
/** The S4 Host after a real drop: the app drives again. */
const S4_DROPPED = { connected: false, unresponsive: false, features: S4_HOST.features }
...
const loopRig = () => {
  ...
  const ctx: DispatchLoopContext = {
    ...
    integrate: async () => ({ merged: [], conflicted: [], failed: [] }) as never,
    reap: async () => true,
...
describe('appTimerTick (N3, D5)', () => {
  ...
  ])('in front of %s, fires the schedule and nudges as it always has', async (_name, status) => {
    const h = loopRig()
    appTimerTick(h.loop, {
      serving: true,
  ...
  it('neither fires nor nudges in front of an unresponsive Host that announced dispatch (review I1)', async () => {
    const h = loopRig()
    const loop = {
      forgetArming: vi.fn(),
      armOnly: vi.fn(() => h.loop.armOnly()),
      fireTick: vi.fn(async () => {}),
      nudge: vi.fn(async () => {})
    }
    appTimerTick(loop, { serving: true, hostDrives: hostSpeaksDispatch(S4_UNRESPONSIVE), log: () => {} })
    expect(loop.fireTick).not.toHaveBeenCalled()
    expect(loop.nudge).not.toHaveBeenCalled()
    expect(loop.armOnly).toHaveBeenCalledTimes(1)
    appTimerTick(loop, { serving: true, hostDrives: hostSpeaksDispatch(S4_DROPPED), log: () => {} })
    expect(loop.fireTick).toHaveBeenCalledTimes(1)
    expect(loop.nudge).toHaveBeenCalledTimes(1)
  })
...
describe('stopRunFromPanel (§5.1, Task 10 carry)', () => {
  const rig = (over: { answer?: () => Promise<{ status: number; body: unknown }> } = {}) => {
    const asked: string[] = []
    const marked: string[] = []
    ...
    return {
      ...
      deps: {
        askHost: (runId: string) => {
          asked.push(runId)
          return over.answer ? over.answer() : Promise.resolve({ status: 200, body: { stopped: true } })
        },
  ...
  it.each([
    ['a 501 (a Host that runs no validations)', async () => ({ status: 501, body: { error: 'no' } })],
    [
      'a rejection',
  ...
  ])('degrades on %s: logs it and kills the run, so its exit reads as a result', async (_n, answer) => {
    const h = rig({ answer })
    await stopRunFromPanel({
      runId: 'r1',
      isValidation: true,
  ...
  it.each([
    ['an S3 Host', S3_HOST],
    ['an S2 Host', S2_HOST]
  ])('in front of %s, marks and kills in the app as before, and asks the Host nothing', async (_n, status) => {
    const h = rig()
    await stopRunFromPanel({
      runId: 'r1',
      isValidation: true,
  ...
  it('still forwards a validation stop to an unresponsive Host that announced dispatch (review I1)', async () => {
    const h = rig()
    await stopRunFromPanel({
      runId: 'r1',
      isValidation: true,
      hostDrives: hostSpeaksDispatch(S4_UNRESPONSIVE),
      ...h.deps
    })
    expect(h.asked).toEqual(['r1'])
    expect(h.stopped).toEqual([])
  ...
  it('degrades after the bound when the unresponsive Host does not answer (review I1)', async () => {
    vi.useFakeTimers()
    try {
      const h = rig({ answer: () => new Promise(() => {}) })
      const done = stopRunFromPanel({
...
describe('appDiscardRunWorktree (R25, C7)', () => {
  ...
  it('makes run-start’s failed coordinator say the fresh worktree could not be removed', async () => {
    ...
    const deps = {
      ...
      readWorker: async () => '',
      now: () => NOW,
      listAccounts: () => [{ id: 'cl1', label: 'claude1', provider: 'claude' as const }],
    ...
    const created = await handleCommand(deps, { sessionId: 'coordinator' }, 'run-create', {
      objective: 'o',
      cwd: '/p',
      auto: true,
    ...
    const r = await handleCommand(deps, { sessionId: 'coordinator' }, 'run-start', { run: runId })
