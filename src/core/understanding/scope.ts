...
export function scopeToStep(x: FeatureExplanation, nodeId: string): ScopedView | null {
  const node = [...x.userFlow, ...x.failureFlows].find((n) => n.id === nodeId)
  if (!node || !isScopable(node)) return null
  const ids = node.evidenceIds ?? []
  return {
    node,
    decisions: x.keyDecisions.filter((d) => overlaps(d.evidenceIds, ids)),
    implementation: x.implementation.filter((i) => overlaps(i.evidenceIds, ids)),
    changes: x.recentChanges.filter((c) => overlaps(c.evidenceIds, ids))
  }
}
