...
export function planLaunch(configs: readonly RunConfig[], rootId: string): LaunchPlan {
  ...
  const expand = (id: string, inheritedAfter: string[], path: string[], heldBy: string | null): string[] => {
    ...
    // static `inheritedAfter`.
    for (const b of c.beforeLaunch ?? []) after.push(...expand(b, after, nextPath, id))
    if (c.type === 'compound') {
      // No step of its own: a compound is not a run, it is ▶ pressed on each member.
      const done: string[] = []
      for (const m of c.members) done.push(...expand(m, after, nextPath, id))
      return done
    }
  ...
  if (!sorted) return { ok: false, reason: 'CYCLE', path: [...steps.keys()] }

  return {
    ok: true,
    steps: sorted.map((configId) => ({ configId, after: steps.get(configId) ?? [] })),
 ...
 *  null when the dependencies cannot be satisfied in any order. */
function topoSort(steps: ReadonlyMap<string, string[]>): string[] | null {
...
export function addableTargets(
  ...
  return configs.filter((c) => {
    ...
    const probe = configs.map((x) => (x.id === hostId ? withRef(host, c.id) : x))
    return planLaunch(probe, hostId).ok
  })
}

/** `host` with one more reference — a member for a compound, a before-launch task for anything else.
 *  Only ever handed to planLaunch, never stored. */
function withRef(host: RunConfig, id: string): RunConfig {
  if (host.type === 'compound') return { ...host, members: [...host.members, id] }
  return { ...host, beforeLaunch: [...(host.beforeLaunch ?? []), id] }
}
