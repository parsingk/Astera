      ...
      do {
        ...
        for (const slot of slots) {
          ...
          try {
            ...
            if (!firstAccount) {
              await gateSlot(slot.taskId, t(c.lang(), 'jobs.gate.assignedAccountUnusable'))
              continue
            }
            ...
            if (run.worktree === undefined) {
              try {
                ...
                // 것이 상태 변경이다).
                await gateSlot(slot.taskId, `Run 워크트리를 만들지 못했습니다: ${String(e)}`)
                continue
              }
            }
            ...
            if (merges.length > 0) {
              ...
              if (integration.kind === 'human') {
                await gateSlot(slot.taskId, integration.reason)
                continue
              }
            ...
            if (reply.status >= 400)
              await gateSlot(slot.taskId, `이 Task 를 시작하지 못했습니다: ${JSON.stringify(reply.body)}`)
          } catch (e) {
