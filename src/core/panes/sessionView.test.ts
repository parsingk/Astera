...
import { openSessionView, sessionViewOf, setSessionView, withoutClosedSessions } from './sessionView'
...
describe('openSessionView', () => {
  ...
  it('seeds a session with no entry from the given default', () => {
    ...
    expect(sessionViewOf(result, 's1')).toBe('conversation')
  })
  ...
  it('is a no-op — same reference back — for a session that already has an entry, even with a different default', () => {
    ...
    expect(sessionViewOf(result, 's1')).toBe('terminal')
  })
  ...
  it('leaves an unrelated session in the map alone', () => {
    ...
    expect(sessionViewOf(result, 's1')).toBe('terminal')
    expect(sessionViewOf(result, 's2')).toBe('conversation')
  })
})

describe('sessionViewOf', () => {
  it("reads 'terminal' for a session with no entry", () => {
    expect(sessionViewOf({}, 'unknown')).toBe('terminal')
  })
})
...
describe('setSessionView', () => {
  ...
  it('keeps two sessions set to different views apart', () => {
    ...
    expect(sessionViewOf(b, 's1')).toBe('conversation')
    expect(sessionViewOf(b, 's2')).toBe('terminal')
  })
