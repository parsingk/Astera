...
import { placeSession } from './place'
import {
  MAX_PANES,
  addTab,
  countLeaves,
...
/** 트리가 불변식을 지키는지. placeSession의 모든 결과에 적용한다 — 이 함수가 불변식 1을 깨뜨린
...
describe('placeSession — 레이아웃이 비어 있을 때', () => {
  it('새 그룹을 만들고 그 그룹을 포커스한다', () => {
    const res = placeSession(null, 's1')
    expect(res.root.kind).toBe('leaf')
    expect(leafOf(res.root, res.paneId!)!.sessionIds).toEqual(['s1'])
    expect(res.splitFellBack).toBe(false)
    assertInvariants(res.root, ['s1'])
  })

  it('분할을 요청해도 쪼갤 대상이 없으므로 그냥 새 그룹이다', () => {
    const res = placeSession(null, 's1', { splitDir: 'row' })
...
describe('placeSession — 활성 그룹에 탭으로 올린다', () => {
  it('activePaneId가 가리키는 그룹에 넣고 활성 탭으로 만든다', () => {
    const { root, ids } = threePane()
    const res = placeSession(root, 'sNew', { activePaneId: ids[2] })
    expect(res.paneId).toBe(ids[2])
    expect(leafOf(res.root, ids[2])!.sessionIds).toEqual(['s3', 'sNew'])
    expect(leafOf(res.root, ids[2])!.activeSessionId).toBe('sNew')
  ...
  it('activePaneId가 없거나 트리에 없으면 firstLeaf에 올린다', () => {
    ...
    for (const active of [null, undefined, 'nope']) {
      const res = placeSession(root, 'sNew', { activePaneId: active })
      expect(res.paneId).toBe(ids[0])
      expect(leafOf(res.root, ids[0])!.sessionIds).toEqual(['s1', 'sNew'])
      assertInvariants(res.root, ['s1', 's2', 's3', 'sNew'])
    }
  })
  ...
  it('원본 트리를 변경하지 않는다', () => {
    ...
    placeSession(root, 'sNew')
    expect(JSON.stringify(root)).toBe(before)
  })
})

describe('placeSession — 이미 트리에 있는 세션 (리뷰에서 잡힌 회귀)', () => {
  it('다른 그룹에 있으면 재삽입하지 않고 그 그룹을 활성화한다', () => {
    const { root, ids } = threePane()
    // 롤링 재개 가드가 새로 spawn하는 대신 이미 열려 있는 live 세션을 그대로 돌려주는 경로
    const res = placeSession(root, 's3', { activePaneId: ids[0] })
    expect(res.paneId).toBe(ids[2]) // 활성 그룹이 아니라 s3가 실제로 있는 그룹
    expect(leafOf(res.root, ids[2])!.activeSessionId).toBe('s3')
  ...
  it('활성 그룹에 이미 있으면 활성 탭으로만 만든다', () => {
    ...
    const res = placeSession(two, 's1', { activePaneId: g.id })
    expect(res.paneId).toBe(g.id)
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['s1', 's2']) // 순서 그대로
    expect(leafOf(res.root, g.id)!.activeSessionId).toBe('s1')
...
describe('placeSession — background (오케스트레이션 워커)', () => {
  it('탭은 올리되 활성 탭을 바꾸지 않는다', () => {
    const { root, ids } = threePane()
    const before = leafOf(root, ids[2])!.activeSessionId
    const res = placeSession(root, 'sWorker', { activePaneId: ids[2], background: true })
    expect(leafOf(res.root, ids[2])!.sessionIds).toEqual(['s3', 'sWorker'])
    expect(leafOf(res.root, ids[2])!.activeSessionId).toBe(before) // 's3' — 바뀌지 않았다
  ...
  it('탭은 트리에 실재한다 — 사용자가 눌러서 열 수 있어야 한다 (C2 회귀 방어)', () => {
    const { root, ids } = threePane()
    const res = placeSession(root, 'sWorker', { activePaneId: ids[1], background: true })
    const g = groupOfSession(res.root, 'sWorker')
    expect(g).not.toBeNull()
    expect(g!.id).toBe(ids[1]) // 활성 그룹에 들어간다
  })

  it('레이아웃이 비어 있으면 그룹을 만들되 활성 패널로 세우지 않는다', () => {
    const res = placeSession(null, 'sWorker', { background: true })
    expect(leaves(res.root)[0].sessionIds).toEqual(['sWorker'])
    expect(res.paneId).toBeNull()
  ...
  it('이미 트리에 있으면 아무것도 하지 않는다 — 활성화도 하지 않는다', () => {
    ...
    const g = createGroup('s1')
    const two = addTab(g, g.id, 's2') // 활성 탭은 s2
    const res = placeSession(two, 's1', { activePaneId: g.id, background: true })
    expect(leafOf(res.root, g.id)!.activeSessionId).toBe('s2')
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['s1', 's2'])
    expect(res.paneId).toBeNull()
    assertInvariants(res.root, ['s1', 's2'])
  })
...
describe('placeSession — replaces (재시작·롤링)', () => {
  it('탭의 자리·순서를 유지한 채 id만 갈아끼우고 포커스는 옮기지 않는다', () => {
    const g = createGroup('s1')
    const three = addTab(addTab(g, g.id, 's2'), g.id, 's3')
    const res = placeSession(three, 'sNew', { activePaneId: g.id, replaces: 's2' })
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['s1', 'sNew', 's3'])
  ...
  it('교체 대상이 활성 탭이었으면 활성 탭도 따라 바뀐다', () => {
    const g = createGroup('s1')
    const two = addTab(g, g.id, 's2') // 활성 탭은 s2
    const res = placeSession(two, 'sNew', { replaces: 's2' })
    expect(leafOf(res.root, g.id)!.activeSessionId).toBe('sNew')
    assertInvariants(res.root, ['s1', 'sNew'])
  })

  it('교체 대상이 트리에 없으면 일반 배치로 떨어진다', () => {
    const { root, ids } = threePane()
    const res = placeSession(root, 'sNew', { activePaneId: ids[0], replaces: 'sGone' })
    expect(res.paneId).toBe(ids[0])
    expect(leafOf(res.root, ids[0])!.sessionIds).toEqual(['s1', 'sNew'])
    assertInvariants(res.root, ['s1', 's2', 's3', 'sNew'])
  })
  ...
  it('레이아웃이 비어 있으면 replaces는 무의미하고 새 그룹이 된다', () => {
    const res = placeSession(null, 'sNew', { replaces: 'sGone' })
    expect(leaves(res.root)[0].sessionIds).toEqual(['sNew'])
    expect(res.paneId).not.toBeNull()
    assertInvariants(res.root, ['sNew'])
  })
})

describe('placeSession — splitDir (Ctrl+\\ 가 예약한 분할)', () => {
  it('새 그룹으로 쪼개고 그 그룹을 포커스한다', () => {
    const g = createGroup('s1')
    const res = placeSession(g, 'sNew', { activePaneId: g.id, splitDir: 'row' })
    expect(countLeaves(res.root)).toBe(2)
    expect(leafOf(res.root, res.paneId!)!.sessionIds).toEqual(['sNew'])
  ...
  it('col 방향도 같다', () => {
    const g = createGroup('s1')
    const res = placeSession(g, 'sNew', { activePaneId: g.id, splitDir: 'col' })
  ...
  it(`그룹이 이미 ${MAX_PANES}개면 활성 그룹에 올리고 폴백을 알린다`, () => {
    ...
    const res = placeSession(root, 'sNew', { activePaneId: active, splitDir: 'row' })
    expect(countLeaves(res.root)).toBe(MAX_PANES) // 상한을 넘지 않는다
    expect(res.paneId).toBe(active)
    expect(leafOf(res.root, active)!.sessionIds).toContain('sNew')
  ...
  it('이미 트리에 있는 세션이 대상 그룹의 유일한 탭이면 쪼개지 않고 활성화만 한다', () => {
    ...
    const g = createGroup('s1')
    const res = placeSession(g, 's1', { activePaneId: g.id, splitDir: 'row' })
    expect(countLeaves(res.root)).toBe(1)
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['s1'])
    expect(res.paneId).toBe(g.id)
    expect(res.splitFellBack).toBe(true)
  ...
  it('이미 다른 그룹에 있는 세션은 그 탭을 새 그룹으로 옮긴다', () => {
    ...
    const res = placeSession(two, 's2', { activePaneId: g.id, splitDir: 'row' })
    expect(countLeaves(res.root)).toBe(2)
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['s1'])
  ...
  it('replaces가 splitDir보다 우선한다 — 재시작은 자리를 이어받는 것이 목적이다', () => {
    const g = createGroup('s1')
    const res = placeSession(g, 'sNew', { activePaneId: g.id, splitDir: 'row', replaces: 's1' })
    expect(countLeaves(res.root)).toBe(1)
    expect(leafOf(res.root, g.id)!.sessionIds).toEqual(['sNew'])
    expect(res.paneId).toBeNull()
    assertInvariants(res.root, ['sNew'])
  })
  ...
  it('activePaneId가 유효하지 않으면 firstLeaf를 쪼갠다', () => {
    const { root, ids } = threePane()
    const res = placeSession(root, 'sNew', { activePaneId: 'nope', splitDir: 'col' })
    expect(countLeaves(res.root)).toBe(4)
    expect(groupOfSession(res.root, 's1')!.id).toBe(ids[0]) // s1은 원래 그룹에 남는다
    assertInvariants(res.root, ['s1', 's2', 's3', 'sNew'])
  })
})
