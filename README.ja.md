...
**プレビュー**
- 開発サーバーのページを Astera の中で、それを直している セッションの隣に見ます — ブラウザタブは
  セッションやファイルと同じペインタブなので、並べる・分割する・ドラッグで動かすが同じようにできます
- リンクの規則はひとつです。どのターミナルでも `localhost` のアドレスをクリックするとプレビュータブが
  開き、それ以外は外部ブラウザで開きます。`Ctrl`/`Cmd` を押しながらクリックすると逆になります
- 実行構成にプレビューのアドレスを書いておくと、その構成が始まるたびにタブが開くか再読み込みされ、
  サーバーが上がるまで待ちます。書いていなくても、実行が出力した最初の `localhost` アドレスを実行タブが
  見つけて開くことを提案します
- ツールバーには戻る・進む・再読み込み、アドレスバー、モバイルからデスクトップまでの画面幅プリセット
  （縦横の切り替え付き）、DevTools、外部ブラウザで開くがあり、右クリックで要素を検査できます
- 自己署名証明書と、カメラや通知などの権限要求は `localhost` に対してだけ許可されます

**Design Mode**
- プレビューのツールバーで **Design Mode** をオンにすると、ページ上の要素を選べます。重ねると強調
  され、クリックするとそこに番号のバッジが付き、横にメモ欄が開きます — 「何を変えますか？」に答え、
  意図を **Change** か **Question** で示します
- メモはタブごとに 20 個まで、番号は付け直しません — 「3 番のように」と書いたメモが最後まで正しく
  なければならないからです。別のルートへ行って戻っても一覧は残り、バッジはそのページで再び表示されます
- **セッションへ送る** は、すべてのメモをひとつの Markdown ブロックとして選んだセッションに貼り付けます
  （セッションが 1 つならそのまま、複数なら選んで）。そのセッションが何かを尋ねている最中なら送らずに
  知らせます。**コピー** は同じブロックをクリップボードに置きます
- 各メモには要素のセレクター・位置・スタイル・HTML、周辺のテキスト（秘密情報は隠して）、そしてその
  要素を切り出したスクリーンショットのパスが添えられます — エージェントは言葉の説明ではなく、その要素
  そのものを見ます
- ページが別の場所へ移動すると Design Mode は自動的にオフになります。`Esc` でも切れます

**エージェントブラウザ (実験)**
- セッションのエージェントが自分専用のプレビュータブを持ち、自分で操作します — このプロジェクトの
  開発サーバーのページを開き、修正したら再読み込みし、アドレスとタイトルを読み、その読み込み以降に
  コンソールとネットワークが残したものを読み返します
- 操作は小さな JavaScript スクリプトで行います — `astera browser js --file check.js` — そのスクリプト
  が使える語彙はそのヘルパーだけです。「画面はまだ描かれているか、コンソールは静かか」を確認して
  ほしいと頼む代わりに、エージェント自身が見て答えます
- そのタブはウィンドウの中の本物のタブです。エージェントのものと表示され、スクリプトが動いている間は
  動作中として見えます。眺めていても、直接クリックしても構いません — 次のスクリプトは、残したままの
  ページを見ます
- **このマシンだけ。** `open()` が受け付けるのは `localhost`・`127.x`・`[::1]`・`*.localhost` と 2 つの
  ワイルドカードアドレスです。それ以外は拒否され、ページ内のリンクや開発サーバーのリダイレクトが外へ
  出ようとしても拒否されます
- セッションごとに同時に 1 スクリプト、1 スクリプトにつき 60 秒です。タブはセッションと一緒に閉じます
- 今は読むだけです — クリック・入力・スクリーンショットは次の段階です
...
ものではないか、エージェントオーケストレーション・作業単位の追跡・エージェントブラウザがすべて無効に
なっています — 3 つのうち 1 つでも有効なら CLI は配置されます。
