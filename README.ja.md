...
**How It Works (実験)**
- How It Works はコードを読まない人のための機能です。その機能が何をするのか、どの順で
  動くのか、何が失敗しうるのか、なぜこう作られたのか、そしてどの部分がどのファイルを読んで
  書かれたのかを併せて記します
- 既定はオフです。設定で **作業単位の追跡** を有効にし、説明を書く **説明生成アカウント**
  を選びます
- 一度分析を指示すると、リポジトリを読んでそのプロジェクトが何をするのかを一覧にします
- セッションがひとつの作業を終えると、その作業が触れた機能の説明が書き直され、サイドバーには
  実際に依頼した文がそのまま変更として残ります
