...
そして十いくつもの工程がある仕事でも、一手ずつ指示する必要はありません — タスクと依存関係を
描いておけば Astera 自身がグラフをたどって進めますし、あるエージェントに別のセッションを
立ち上げさせ、タスクを渡して同梱の CLI での報告を待たせることもできます。
...
**外観**
- テーマ 6 種 — Vega、Orion、Umbra、Aurora、Antares、Quasar。カードがそれぞれ自分のパレットで自身を
  描くので、名前ではなく見た目で選べます
...
### 1. Jobs サイドバーから実行 — Astera が調整

この方式はアプリが直接管理します。

1. プロジェクトが git リポジトリで、ブランチがチェックアウトされていることを確認します。
2. Jobs サイドバーで **新しい作業** を押し、**目標**・**エージェント**・**同時実行** と、必要なら
   **予約実行** を設定して **作る** を押します。
3. ジョブの詳細画面で **Task を追加** を押し、**タイトル** と **指示** を書いて **先行 Task** を
   選びます。必要なら **アカウント**、完了を検証する実行構成、別のエージェントによるレビューも
   設定します — レビューは必ず別ベンダーで動きます。
4. すべての Task を組み終えてから **実行** を押します。ジョブを作成したり Task を追加したり
   するだけでは開始されません。予約ジョブでは、**実行** は回をすぐに開始するのではなく、
   スケジュールを有効にします。

Astera は依存関係が完了した Task だけを開始します。その後は **同時実行** の値で変わります。2 以上
なら（既定は 3）Task ごとに自分の worktree ができ、後続の Task を始める前に完了したものをジョブ
自身の worktree にマージし、コンフリクトがあればエージェントに解決を任せます。1 なら Task が
1 つの worktree を順に引き継ぐので、マージするものが生じません。どちらでも、完了したジョブは
プロジェクトで現在チェックアウトしているブランチへ自動ではマージされません。結果を取り込む準備が
できたら、詳細画面で **マージ** を押してください。全体の流れは
[Job の詳しいライフサイクル](docs/jobs.md)にあります（現在は韓国語のみ）。
...
スキルは `/astera-orchestration` として明示的に呼び出すこともできます。コーディネーターは Run と
Task を作成し、*別*ベンダーを含むワーカーセッションに作業を割り振り、完了・依存関係・質問・
エスカレーションを待ちます。ワーカーは同梱の `astera` CLI で報告します。Run の作業ディレクトリが
開いているプロジェクトと一致すれば Jobs サイドバーにも表示されますが、何を割り振るかは Astera の
自動スケジューラーではなくコーディネーターが決めます。
...
<div align="center">
<img src="assets/jobs.gif" width="820" alt="図: ジョブのタスクが依存関係のグラフとして描かれ、準備できた 2 つが別々のベンダーで同時に始まり、テストがひとつの完了を証明し、完了した Task の worktree がジョブの worktree にマージされ、ジョブが自分では決められない判断が人を待つ" />
</div>

そしてその様子がそのまま見えます: 開いているプロジェクトのジョブがサイドバーに並び、タスクは
依存関係のグラフとして、起きたことはタイムラインとして。どのベンダーがどのタスクをどれだけの時間
つかんでいるかも見えます。開始・停止・再試行、人への問いかけ、待っている判断への回答 — すべて
グラフのノードから行えます。

コーディネーター CLI のリファレンス全体を自分で読むには:
