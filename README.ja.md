...
**How It Works (実験)**
- How It Works はコードを読まない人のための機能です。始めるまでは何も記録されません —
  `/astera-task` にやりたいことを書くと、そこから一行が始まり、メッセージが何度やり取りされても、
  承認でも訂正でも途中の方針転換でも、その作業が終わるまでは同じ一行のままです
...
- Run ひとつは `/astera-task` なしでもそれだけでひとつの行になります。Jobs のサイドバーから
  始めたか、エージェントが `astera-orchestration` スキルで始めたかは問いません
...
- 追跡を有効にしたあとの作業だけを記録します。それ以前は読みません — コードも、
  過去のセッションも、過去の Run も
- 目標を満たしたとエージェントが判断すると、その行が閉じます。画面から自分で閉じることも
  でき、終わらないまま残った作業はそこで待ち続けます
- 行が閉じると数分後に自分で埋まります。書けなかったときは理由を示し、もう一度試せます
- 行を開くと、何が変わったのか、使う人が何に気づくのか、どの順で動くのか、そのとき何を
  判断したのか、そしてコードのどこにあるのかが書かれています
- エージェントが確認したと報告した内容は、あくまで報告どおりに示されるだけで、アプリが
  自分で検証した結果ではありません
