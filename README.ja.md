...
これらはすべて設定 1 つに掛かっています — 設定の**エージェントオーケストレーション**。有効にすると
Jobs のサイドバーも付いてきますし、そこで組むジョブにはほかに必要なものはありません: タスクを挙げ、
何が何を待つかを決めて、始めるだけです。

ジョブとは、依存関係でつながったタスクのまとまりで、2 つのベンダーにまたがって進みます。動かし方は
2 つあります:

- **Astera が進めます。** アプリでジョブを組んでおけば — タスク、何が何を待つか、同時にいくつまで
  — そのまま動きます: 依存関係が片づいたタスクだけを起動し、後続のタスクが始まる前に終わった
  worktree をマージし戻し、コンフリクトは人に押し付けずエージェントに任せます
- **エージェントに進めさせることもできます。** コーディネーターのセッションがワーカーのセッションに
  タスクを割り振り — *別*ベンダーのワーカーにも — 完了・依存関係・質問・エスカレーションを
  待ちます。ワーカーは同梱の `astera` CLI で報告します
...
<div align="center">
<img src="assets/jobs.gif" width="820" alt="図: ジョブのタスクが依存関係のグラフとして描かれ、準備できた 2 つが別々のベンダーで同時に始まり、テストがひとつの完了を証明し、終わった worktree がマージし戻され、ジョブが自分では決められない判断が人を待つ" />
</div>
...
エージェントが調整するジョブなら、その設定を有効にしてからセッションを開始してください。
そのセッションの `PATH` に `astera` CLI が入り、使い方を説明するスキルも渡されるので、作業を
調整してほしいと伝えるだけで足ります。リファレンス全体を自分で読むには:
