...
**会話画面**
- セッションはターミナルとしても、ふつうのチャット UI としても開けます。どちらで開くかはセッションを
  作るときに決まり、あとから変わりません。新しいセッションの既定は **設定 → 一般** で決めます
- 会話セッションでは選択肢と権限確認がカードで届きます。モデルと権限モードは入力欄の横で選びます
...
**スマート再開 (実験)**
- 既定ではオフです。**設定 → エージェント → セッション再開方法** で、CLI 従来の Resume とこちらのどちらかを
  選びます
...
**通過するまで自動修正**
- Job のビルドやテストが失敗したり、レビューがブロッキングな指摘を出したら、その内容を
  ワーカーに差し戻して直させ、もう一度検査します — 完了条件を満たすか、再試行の上限に
  達するまで
- アカウントの切り替え、一時停止、アプリの再起動をまたいでも続くので、Job は最初から
  やり直しになりません
...
**GitHub**
- worktree の一覧に PR の状態が表示されます
- ベースにないコミットを持つ worktree は PR の作成を提案します。ダイアログひとつでブランチをプッシュし、
  その worktree が分岐した元のブランチに対して PR を開きます
- すでにある `gh` のログインを借ります。トークンは保存しません

**How It Works (実験)**
- How It Works はコードを読まない人のための機能です。`/astera-task` スキルでやりたいことを書くと、
  そこから記録の準備が始まり、作業が終わると内容を読み取って書き起こします
- `/astera-task` は、オーケストレーションのものと同じくアプリがアカウントに入れるスキルです。
  開始を伝え、あなたのメッセージが何度やり取りされてもその作業を続け、目標を満たしたら終了を
  伝えるようエージェントに指示します — 目標を書けば、あとはエージェントがやります
- Claude Code と Codex のネイティブ `/goal` も同じように記録されます。ただし Claude は作業が
  終わると記録が自動で完了になり、Codex は自分で完了にする必要があります
- 1 セッションで記録できる作業は 1 つです。すでに記録中の作業があるときに来た依頼は、先に前の
  記録を完了してほしいと知らせ、完了すると次の記録が始まります
- Jobs の Run（`/astera-orchestration`）も同じように読み取られ、記録されます。ただし読み取りは
  Run が終わってから始まります
- ファイルが何も変わらなかった作業は行を残しません。宣言しただけで話しただけなら、書くことが
  ありません
- 既定はオフです。**設定 → How It Works** で **作業単位の追跡** を有効にし、記録を書く **説明生成アカウント**
  を選びます。すでに開いているセッションには適用されません — 新しいセッションから有効に
  なります
- 設定でこの機能を有効にしたあとの作業だけを記録します

**Design Mode**
- ウェブページタブで **Design Mode** をオンにすると、ページ上の要素を選べます。重ねると強調され、
  クリックするとそこに番号のバッジが付き、横にメモ欄が開きます。「何を変えますか？」に答え、意図を
  **Change** か **Question** で示します
- メモはタブごとに 20 個まで、番号は付け直しません。
- **セッションへ送る** は、すべてのメモをひとつの Markdown ブロックとして選んだセッションに貼り付けます。
- 各メモには要素のセレクター・位置・スタイル・HTML、周辺のテキスト（秘密情報は隠して）、そしてその
  要素を切り出したスクリーンショットのパスが添えられます。エージェントは言葉の説明ではなく、要素
  そのものを見ます
- ページが別の場所へ移動すると Design Mode は自動的にオフになります。

**エージェントブラウザ (実験)**
- セッションのエージェントがエージェント専用のウェブページタブを持ち、自分で操作します。開発サーバーの
  ページを開き、修正したら再読み込みし、アドレスとタイトルを読み、その読み込み以降にコンソールと
  ネットワークが残したものを読み返します。
- 「画面はまだ描かれているか、コンソールは静かか」を確認してほしいと頼む代わりに、エージェント自身が
  見て答えられます。
- エージェントのウェブページタブが受け付けるのは `localhost`・`127.x`・`[::1]`・`*.localhost` と 2 つの
  ワイルドカードアドレスです。それ以外は拒否され、ページ内のリンクや開発サーバーのリダイレクトが外へ
  出ようとしても拒否されます
- タブはセッションと一緒に閉じます
- ページを文章として読み（スナップショット）、画像として見て（スクリーンショット）、要素をクリックし、値を入力し、キーを押し、待ちます
- 既定ではオフです。設定で**エージェントブラウザ**をオンにすると、他の 2 つの機能と同じように
  `astera-browser` スキルをアカウントにインストールします。すでに開いているセッションには適用され
  ません。新しいセッションから有効になります
...
Jobs は有効にする必要がなく、いつでもあります。Jobs サイドバーはアプリの一部です。ジョブは Claude と
Codex で実行できるタスクの依存関係グラフで、動かし方は 3 つあります。
...
2. **新しい作業** を押し、**目標**・**コーディネーターアカウント**・**同時実行**を設定し、
   必要ならスケジュールを追加します。
3. Task ごとに指示、1 つ以上のワーカーアカウント、先行 Task を設定します。ビルド・テスト・
   別ベンダーのレビューを完了条件にできます。**通過するまで自動修正**をオンにすると、検査が
   失敗したりレビューがブロッキングな指摘を出したときに、その内容をワーカーに差し戻して
   直させ、もう一度検査します — 完了条件を満たすか、再試行の上限に達するまで。
...
Astera が開始するセッションはすべて、`PATH` に `astera` CLI が追加され、`astera-orchestration`
スキルも渡されます。たとえば次のように頼めます。
...
Run は Jobs サイドバーにも表示され、**作業単位の追跡** を有効にしていれば、終わったときに
How It Works にひとつずつ行として残ります — 自分で始めた Run と同じです。

スキルはセッション開始時に読み込まれるため、アップグレード前から開いていたセッションにはありません。
...
`astera` が `PATH` にない場合は、環境変数 `ASTERA_CLI` のパスを使います（bash・zsh では
`"$ASTERA_CLI"`、PowerShell では `$env:ASTERA_CLI`）。値が空なら、そのセッションは Astera が開始した
ものではありません。Astera が開始するセッションはすべて CLI を受け取ります。

### 3. ターミナル、スクリプト、エージェントから `astera` コマンドで動かす

同じ Job を、普通のターミナル、CI パイプライン、エージェントから一覧し、開始し、待ち、答えられます。
**設定 → エージェント → Command line tool (astera)** から一度インストールします。Astera が開始した
セッションには最初から入っているので、その中のエージェントも同じコマンドを呼べます。アプリを終了しても
コマンドは答え続けます。作業を持つバックグラウンドプロセス **Astera Host** がウィンドウより長く生きる
ためです。Host が動いていなければ `astera host start` で起動します。

```bash
astera --help                     # すべてのコマンドの使い方 (ダッシュなしの astera help はエージェント向けガイド)
astera jobs list --human          # JSON の代わりに読みやすい列
run=$(astera jobs run --id job_123 | jq -r '.data.id')
astera runs wait --id "$run"      # 0 完了、8 人が必要、10 失敗、7 期限切れ
