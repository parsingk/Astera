...
> **넣었다** — 3번(무엇을 포기하나) 끝의 한 문장으로. 문단이 길어서 뺀 것이지 사실이 틀려서 뺀 것이
> 아니고, 승계되는 것이 바로 3번이 말하는 그 대가라 붙을 자리도 거기다.
>
...
머지 시점에 다음으로 넘긴 다섯 항목 중 셋은 **이미 없었다**. 자동 재시도를 걷어내고 사람이 누르는
버튼으로 바꾸면서(F5 재작성) 두 시도의 꼬리를 합치는 코드 자체가 사라졌고, 그와 함께 합친 꼬리의
4053자 초과도, 미번역 `---`/`[attempt N]` 이름표도, 두 번째 시도에 stderr 가 없을 때 첫 시도의 이유를
물려받던 것도 같이 사라졌다. 재시도는 이제 새 어댑터를 만들고, `state()` 는 그 어댑터의 상태를 그대로
돌려준다. 남은 둘과, 절을 빼면서 생긴 하나를 여기서 처리했다.

### F6 — 앱 진입 게이트는 설치 여부를 묻는다

`App.tsx` 가 폴더 없이 `system.checkCli()` 를 불러 그 결과로 **앱 전체**를 막고 있었다. 그것은
"이 앱의 작업 디렉터리에서 `--version` 이 돌았나" 라는 질문이고, 답이 아니오인 이유 중 하나가 바로
이 조각이 다룬 그 상황이다 — 설치는 멀쩡한데 PATH 앞의 버전 관리자가 **그 폴더에서** 실행을 거절하는
것. 그러면 사람이 고른 적도 없는 폴더의 파일 하나 때문에 앱 전체가 잠기고, 그 화면이 내미는 설치
버튼은 없는 것을 설치할 수 없으므로 아무것도 고치지 못한다. 최종 리뷰가 새 세션 창의 시작 게이트에서
잡아낸 것과 같은 실수가 한 화면 위에 남아 있었다.

- 게이트와 `anyCliOk` 는 `system.checkCliInstalled()`(기계의 PATH, 폴더 무관)를 본다.
- `system.checkCli()` 는 남는다 — 설정 화면의 버전 문자열이 그것이고, 거기서는 그게 맞는 답이다.
- `CliMissingScreen` 의 설치 후 재검사도 같은 질문으로 바꿨다. 게이트와 다른 질문을 쓰면, 설치는
  됐는데 이 폴더에서 안 도는 CLI 앞에서 화면이 "아직 없다" 며 이미 있는 것을 또 설치하자고 한다.

**대가**: 설치돼 있지만 **어디서도** 안 도는 CLI(망가진 설치)는 이제 이 화면에 닿지 않는다. 앱 안에서
재설치를 누를 자리가 `CliMissingScreen` 하나뿐이므로, 그 경우의 앱 내 복구 경로가 없어진다. 대신 새
세션 창이 폴더마다 도구 자신의 에러 줄을 보여주므로 진단은 오히려 낫다. 설정 화면에 설치 버튼을
두는 것이 다음 자리다 — **F9 에서 했다.**

### F9 — 설정 화면에서도 설치한다

F6 이 만든 구멍을 막는다. 설정 → 정보의 CLI 두 줄이 버전만 말하고 아무 버튼이 없었는데, 하필 그 줄이
"감지 안 됨" 이라고 말하는 사람이 앱 안에서 할 수 있는 일이 없는 사람이다.

- `hooks/useCliInstall.ts`(신규): 설치 호출·스트리밍 로그·성공 후 재검사·`unseen` 갈래. 전부
  `CliMissingScreen` 이 쥐고 있던 것을 그대로 옮겼다. 두 벌로 두면 구독과 `unseen` 처리를 두 군데서
  맞춰야 한다.
- `components/CliInstallRows.tsx`(신규): 설정의 두 줄. 이미 잡히는 CLI 에도 버튼을 낸다 — 벤더의 설치
  명령이 곧 업그레이드 명령이라, 라벨만 "다시 설치" 로 바꿔 정직하게 부른다.
- 성공은 말로 한다(`settings.info.cliInstallDone`). 잘 되던 것을 다시 설치하면 버전 문자열이 그대로일
  수 있고, 안 바뀐 줄은 "눌렀는데 아무 일도 안 났다" 로 읽힌다. 전체 화면 쪽은 화면이 사라지는 것이
  답이라 이 문장이 필요 없었다.
- 이 운영체제용 설치 명령이 없으면 버튼도 없다. 전체 화면은 그것을 한 문장으로 설명하지만, 설정 줄에
  긴 문장을 넣느니 예전과 똑같이 보이는 편이 낫다.

### F7 — 롤 승계 한 문장을 대가 옆에 되돌린다

"얼마나 / 무엇이 바뀌나" 절을 통째로 뺄 때 같이 사라졌던 사실이다. §4 F5 의 주석을 볼 것.

### F8 — 확인 창이 다이얼로그로 읽힌다

`role="dialog"`·`aria-modal="true"`·`aria-labelledby`(제목). 동의를 받는 창이라 다른 모달보다 이유가
크다 — 읽는 사람이 무엇을 포기하는지 읽고 누르는 것이 이 창의 전부인데, 화면 낭독기가 문단 더미로
지나가면 그 전제가 무너진다. `UpdateGate.tsx` 는 끼어드는 알림이라 `alertdialog` 이고, 이쪽은 선택이라
`dialog` 다. 나머지 `.modal` 들에는 아직 없다.
