...
4. **얼마나 오래 / 무엇이 바뀌나** — 이 세션이 사는 동안만. 다른 세션과 터미널은 그대로고, **파일도 설정도
   건드리지 않는다.** 되돌리는 법은 "이 세션을 닫고 새로 시작".
5. **제대로 된 해결** — `package.json` 을 고치면 이 창은 다시 뜨지 않는다.
