...
<div align="center">
<img src="assets/jobs.gif" width="820" alt="다이어그램: Job의 작업들이 의존성 그래프로 그려지고, 준비된 두 작업이 서로 다른 벤더에서 동시에 시작되며, 테스트가 하나의 완료를 증명하고, 끝난 worktree들이 되돌려 합쳐진 뒤, 스스로 정할 수 없는 결정이 사람을 기다린다" />
</div>
...
에이전트가 조율하는 Job이라면, 그 설정을 켠 뒤에 세션을 시작하세요. 그 세션의 `PATH`에 `astera` CLI가
올라가고 사용법을 설명하는 스킬이 함께 주어지므로, 작업을 조율하라고 그냥 말하면 됩니다. 전체
레퍼런스를 직접 보려면:
