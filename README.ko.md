...
**How It Works (실험)**
- How It Works는 코드를 읽지 않는 사람을 위한 기능입니다. 시작하기 전엔 아무것도 기록되지
  않습니다 — `/astera-task`로 하려는 일을 적으면 그때부터 한 줄이 시작되고, 메시지가 몇 번
  오가든, 승인이든 정정이든 마음이 바뀌든 그 작업이 끝날 때까지는 같은 한 줄입니다
...
- Run 하나는 `/astera-task` 없이도 그 자체로 한 줄입니다. Jobs 사이드바에서 시작했든 에이전트가
  `astera-orchestration` 스킬로 시작했든 같습니다
...
- 추적을 켜고 난 뒤의 작업만 기록합니다. 그 전은 읽지 않습니다 — 코드도, 지난 세션도,
  지난 Run 도
- 목표를 다했다고 에이전트가 판단하면 그 줄을 닫습니다. 화면에서 직접 닫을 수도 있고,
  끝을 맺지 못한 작업은 그렇게 화면에 남아 기다립니다
- 줄이 닫히면 몇 분 뒤에 스스로 채워집니다. 쓰지 못했으면 그 사유를 보이고 다시 만들 수
  있게 합니다
- 줄을 열면 무엇이 달라졌는지, 쓰는 사람이 무엇을 느끼게 되는지, 어떤 순서로 동작하는지,
  그때 무슨 판단을 했는지, 그리고 코드 어느 자리인지가 적혀 있습니다
- 에이전트가 확인했다고 보고한 내용은 보고된 그대로 보여줄 뿐, 앱이 직접 검증한 결과가
  아닙니다
- 읽기만 합니다. 여기서 하는 어떤 일도 코드를 바꾸지 않습니다
