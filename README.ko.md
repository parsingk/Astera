...
**대화 화면**
- 세션은 터미널로도, 일반 채팅 UI 로도 열 수 있습니다. 어느 쪽인지는 세션을 만들 때 정하고
  이후에는 바뀌지 않습니다. 새 세션이 무엇으로 열릴지는 **설정 → 일반**에서 정합니다
- 대화창 세션은 선택지와 권한 확인이 카드로 옵니다. 작성란 옆에서 모델과 권한 모드를 고릅니다
...
**통과할 때까지 자동 수정**
- Job의 빌드·테스트가 실패하거나 검토가 막는 지적을 내면, 그 내용을 워커에게 돌려보내
  고치게 하고 다시 검사합니다 — 완료 조건을 통과하거나 재시도 한도에 닿을 때까지
- 계정 전환과 일시 정지, 앱 재시작을 넘겨도 이어지므로 Job이 처음부터 다시 시작하지
  않습니다
...
**Design Mode**
- 웹페이지 탭의 **Design Mode**를 켜면 페이지 위에서 요소를 고를 수 있습니다. 올리면 강조되고,
  누르면 그 자리에 번호 배지가 붙고 옆에 메모 상자가 열립니다. "무엇을 바꿀까요?"에 답하고 의도를
  **Change** 또는 **Question**으로 표시합니다
- 메모는 탭마다 20개까지 쌓이고, 번호는 다시 매기지 않습니다.
- **세션에 보내기**는 모든 메모를 하나의 마크다운 블록으로 고르는 세션에 붙여 넣습니다. 
- 메모마다 요소의 선택자·위치·스타일·HTML, 주변 텍스트(비밀 값은 가려서), 그리고 그 요소를 잘라 낸
  스크린샷 파일의 경로가 함께 갑니다. 에이전트가 말로 된 설명 대신 요소를 직접 봅니다
- 페이지가 다른 곳으로 이동하면 Design Mode는 스스로 꺼집니다.

**에이전트 브라우저 (실험)**
- 세션의 에이전트가 에이전트 전용 웹페이지 탭을 갖고 직접 조작합니다. 개발 서버의 페이지를
  열고, 고친 뒤 새로고침하고, 주소와 제목을 읽고, 그 로드 이후 콘솔과 네트워크가 남긴 것을
  되읽습니다.
- "화면이 여전히 그려지는지, 콘솔이 깨끗한지"를 봐 달라고 부탁하는 대신 에이전트가 직접 보고 답할 수 있습니다.
- 에이전트 웹페이지 탭은 `localhost`, `127.x`, `[::1]`, `*.localhost`와 두 와일드카드 주소를
  받습니다. 그 밖은 거절되고, 페이지의 링크나 개발 서버의 리다이렉트가 밖으로 나가려는 것도
  거절됩니다
- 탭은 세션과 함께 닫힙니다
- 페이지를 글로 읽고(스냅샷) 그림으로 보며(스크린샷), 요소를 누르고 값을 채우고 키를 누르며 기다립니다
...
2. **새 작업**을 눌러 **목표**와 **코디네이터 계정**, **동시 실행**을 정하고 필요하면 예약을
   추가합니다.
3. Task마다 지시와 하나 이상의 워커 계정, 선행 Task를 정합니다. 빌드·테스트·다른 벤더의 검토를
   완료 조건으로 연결할 수 있습니다. **통과할 때까지 자동 수정**을 켜 두면, 검사가 실패하거나
   검토가 막는 지적을 냈을 때 그 내용을 워커에게 돌려보내 고치게 하고 다시 검사합니다 —
   완료 조건을 통과하거나 재시도 한도에 닿을 때까지.
