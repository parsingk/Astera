...
**미리보기**
- 개발 서버의 페이지를 Astera 안에서, 그 페이지를 고치는 세션 옆에 봅니다 — 브라우저 탭은 세션·파일
  탭과 같은 페인 탭이라 나란히 두고, 나누고, 끌어 옮길 수 있습니다
- 링크 규칙은 하나입니다. 어느 터미널에서든 `localhost` 주소를 누르면 미리보기 탭이 열리고, 그 밖의
  주소는 외부 브라우저로 갑니다. `Ctrl`/`Cmd`를 누른 채 클릭하면 반대로 동작합니다
- 실행 구성에 미리보기 주소를 적어 두면 그 구성이 시작될 때 탭이 열리거나 새로고침되며, 서버가 뜰
  때까지 기다립니다. 적어 두지 않아도, 실행이 출력한 첫 `localhost` 주소를 실행 탭이 알아채 열어 줍니다
- 툴바에는 뒤로·앞으로·새로고침, 주소 표시줄, 화면 폭 프리셋(모바일부터 데스크톱까지, 가로세로 전환),
  DevTools, 외부 브라우저로 열기가 있고, 오른쪽 클릭으로 요소를 검사합니다
- 자체 서명 인증서와 카메라·알림 같은 권한 요청은 `localhost` 에서만 허용됩니다

**Design Mode**
- 미리보기 툴바의 **Design Mode**를 켜면 페이지 위에서 요소를 고를 수 있습니다. 올리면 강조되고,
  누르면 그 자리에 번호 배지가 붙고 옆에 메모 상자가 열립니다 — "무엇을 바꿀까요?"에 답하고 의도를
  **Change** 또는 **Question**으로 표시합니다
- 메모는 탭마다 20개까지 쌓이고, 번호는 다시 매기지 않습니다 — "3번처럼"이라고 쓴 메모가 끝까지
  참이어야 하기 때문입니다. 다른 경로로 갔다 와도 목록은 남고 배지는 그 페이지에서 다시 보입니다
- **세션에 보내기**는 모든 메모를 하나의 마크다운 블록으로 고르는 세션에 붙여 넣습니다(세션이 하나면
  바로, 여럿이면 골라서). 그 세션이 무언가를 묻고 있으면 보내지 않고 알려 줍니다. **복사**는 같은
  블록을 클립보드에 둡니다
- 메모마다 요소의 선택자·위치·스타일·HTML, 주변 텍스트(비밀 값은 가려서), 그리고 그 요소를 잘라 낸
  스크린샷 파일의 경로가 함께 갑니다 — 에이전트가 말로 된 설명 대신 그 요소를 직접 봅니다
- 페이지가 다른 곳으로 이동하면 Design Mode는 스스로 꺼집니다. `Esc`로도 끕니다

**에이전트 브라우저 (실험)**
- 세션의 에이전트가 자기 미리보기 탭을 갖고 직접 조작합니다 — 이 프로젝트 개발 서버의 페이지를
  열고, 고친 뒤 새로고침하고, 주소와 제목을 읽고, 그 로드 이후 콘솔과 네트워크가 남긴 것을
  되읽습니다
- 조작은 작은 JavaScript 스크립트로 합니다 — `astera browser js --file check.js` — 그 스크립트가
  쓸 수 있는 어휘는 그 헬퍼들뿐입니다. "화면이 여전히 그려지는지, 콘솔이 깨끗한지"를 봐 달라고
  부탁하는 대신 에이전트가 직접 보고 답합니다
- 그 탭은 창 안의 실제 탭입니다. 에이전트의 것으로 표시되고 스크립트가 도는 동안 바쁨으로
  보입니다. 지켜봐도 되고 직접 눌러 봐도 됩니다 — 다음 스크립트는 남겨 둔 그대로의 페이지를 봅니다
- **이 컴퓨터만.** `open()`은 `localhost`, `127.x`, `[::1]`, `*.localhost`와 두 와일드카드 주소를
  받습니다. 그 밖은 거절되고, 페이지의 링크나 개발 서버의 리다이렉트가 밖으로 나가려는 것도
  거절됩니다
- 세션당 한 번에 한 스크립트, 스크립트당 60초입니다. 탭은 세션과 함께 닫힙니다
- 아직은 읽기만 합니다 — 클릭·입력·스크린샷은 다음 단계입니다
- 기본값은 꺼짐입니다. 설정에서 **에이전트 브라우저**를 켜면 다른 두 기능과 마찬가지로
  `astera-browser` 스킬을 계정에 설치합니다. 이미 열려 있는 세션에는 적용되지 않습니다 — 새
  세션부터 동작합니다
