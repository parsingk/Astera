...
**How It Works (실험)**
- How It Works는 코드를 읽지 않는 사람을 위한 기능입니다. 작업 하나가 끝날 때마다 무엇을
  어떻게 바꿨는지가 한 줄씩 쌓입니다 — 세션의 작업도, Jobs 의 Run 하나도 같은 한 줄입니다
- 기본값은 꺼짐입니다. 설정에서 **작업 단위 추적**을 켜고, 기록을 쓸 **설명 생성 계정**을
  고릅니다
- 추적을 켜고 난 뒤의 작업만 기록합니다. 그 전은 읽지 않습니다 — 코드도, 지난 세션도,
  지난 Run 도
- 작업이 닫히는 순간 줄이 서고, 몇 분 뒤에 스스로 채워집니다. 쓰지 못했으면 그 사유를
  보이고 다시 만들 수 있게 합니다
